"""
custom_cnn_30k_model.py

Custom lightweight convolutional neural network (~29.8 K trainable parameters)
for RF modulation classification from scaleogram images.

Architecture
------------
    Conv2d(3→10, bias=False)   → BN → ReLU → MaxPool     112×112
    Conv2d(10→20, bias=False)  → BN → ReLU → MaxPool      56×56
    Conv2d(20→32, bias=False)  → BN → ReLU → MaxPool      28×28
    Conv2d(32→36, bias=False)  → BN → ReLU → MaxPool      14×14
    AdaptiveAvgPool(3×3)                                    3×3×36 = 324
    Dropout(0.3)
    Linear(324→34) → ReLU → Dropout(0.3)
    Linear(34→9)

Wider channel progression (10→20→32→36) compared to the 25 K model
(8→16→32→32) provides greater feature capacity while staying under
30,000 trainable parameters.  Conv layers use bias=False since
...
# ---------------------------------------------------------------------------
# Custom CNN model  (~29,759 trainable parameters)
# ---------------------------------------------------------------------------
class CustomCNN30K(nn.Module):
    """Lightweight 4-layer CNN (≤30 K params) for 224×224 RGB scaleogram
    classification.  Channel widths are widened relative to CustomCNN25K
    (10→20→32→36 vs 8→16→32→32) and Conv2d bias is disabled (BatchNorm
    ...
    def __init__(self, num_classes: int = 9):
        ...
        self.features = nn.Sequential(
            # Block 1: 224×224×3 → 112×112×10
            nn.Conv2d(3, 10, kernel_size=3, padding=1, bias=False),
            nn.BatchNorm2d(10),
            nn.ReLU(inplace=True),
            nn.MaxPool2d(2),

            # Block 2: 112×112×10 → 56×56×20
            nn.Conv2d(10, 20, kernel_size=3, padding=1, bias=False),
            nn.BatchNorm2d(20),
            nn.ReLU(inplace=True),
            nn.MaxPool2d(2),

            # Block 3: 56×56×20 → 28×28×32
            nn.Conv2d(20, 32, kernel_size=3, padding=1, bias=False),
            nn.BatchNorm2d(32),
            nn.ReLU(inplace=True),
            nn.MaxPool2d(2),

            # Block 4: 28×28×32 → 14×14×36
            nn.Conv2d(32, 36, kernel_size=3, padding=1, bias=False),
            nn.BatchNorm2d(36),
            nn.ReLU(inplace=True),
            nn.MaxPool2d(2),

            # Reduce to fixed 3×3 spatial size → 324-dim vector
...
def plot_training_curves(history, save_dir):
    ...
    ax.set_xlabel("Epoch")
    ax.set_ylabel("Loss")
    ax.set_title("Custom CNN 30K — Training and Validation Loss")
    ax.legend()
    ax.grid(True)
    fig.tight_layout()
    fig.savefig(save_dir / "custom_cnn_30k_loss_curves.png", dpi=150)
    plt.close(fig)
    ...
    ax.set_title("Custom CNN 30K — Training and Validation F1 Score")
    ax.legend()
    ax.grid(True)
    fig.tight_layout()
    fig.savefig(save_dir / "custom_cnn_30k_f1_curves.png", dpi=150)
    plt.close(fig)
...
def main():
    ...
    # --- Model ---
    model = CustomCNN30K(num_classes=NUM_CLASSES)
    total_params = count_parameters(model)
    print(f"\nCustom CNN 30K — trainable parameters: {total_params:,}")
    assert total_params <= 30_000, (
        f"Model has {total_params:,} parameters, exceeding the 30,000 limit"
    )
    ...
    # --- Save model ---
    model_path = SCRIPT_DIR / "custom_cnn_30k_rf_mod_classifier.pth"
    ...
    plot_confusion_matrix(
        ...
        f"Custom CNN 30K — Training Set Confusion Matrix  (F1={train_f1_final:.4f})",
        SCRIPT_DIR / "custom_cnn_30k_confusion_matrix_train.png",
    )
    ...
    plot_confusion_matrix(
        ...
        f"Custom CNN 30K — Validation Set Confusion Matrix  (F1={val_f1_final:.4f})",
        SCRIPT_DIR / "custom_cnn_30k_confusion_matrix_val.png",
    )
