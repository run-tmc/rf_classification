...
Custom lightweight convolutional neural network (~98 K trainable parameters)
for RF modulation classification from scaleogram images.

Architecture
------------
    Conv2d(3→16)  → BN → ReLU → MaxPool     112×112
    Conv2d(16→32) → BN → ReLU → MaxPool      56×56
    Conv2d(32→64) → BN → ReLU → MaxPool      28×28
    Conv2d(64→64) → BN → ReLU → MaxPool      14×14
    AdaptiveAvgPool(3×3)                       3×3×64 = 576
    Dropout(0.3)
    Linear(576→64) → ReLU → Dropout(0.3)
    Linear(64→9)
...
# ---------------------------------------------------------------------------
# Custom CNN model  (~98,377 trainable parameters)
# ---------------------------------------------------------------------------
class CustomCNN(nn.Module):
    """Lightweight 4-layer CNN for 224×224 RGB scaleogram classification."""
    ...
    def __init__(self, num_classes: int = 9):
        ...
        self.features = nn.Sequential(
            ...
            # Block 4: 28×28×64 → 14×14×64
            nn.Conv2d(64, 64, kernel_size=3, padding=1),
            nn.BatchNorm2d(64),
            nn.ReLU(inplace=True),
            nn.MaxPool2d(2),

            # Reduce to fixed 3×3 spatial size → 576-dim vector
...
def plot_training_curves(history, save_dir):
    ...
    ax.set_xlabel("Epoch")
    ax.set_ylabel("Loss")
    ax.set_title("Custom CNN — Training and Validation Loss")
    ax.legend()
    ax.grid(True)
    fig.tight_layout()
    fig.savefig(save_dir / "custom_cnn_loss_curves.png", dpi=150)
    plt.close(fig)
    ...
    ax.set_title("Custom CNN — Training and Validation F1 Score")
    ax.legend()
    ax.grid(True)
    fig.tight_layout()
    fig.savefig(save_dir / "custom_cnn_f1_curves.png", dpi=150)
    plt.close(fig)
...
def main():
    ...
    print(f"\nCustom CNN — trainable parameters: {total_params:,}")
    assert total_params <= 100_000, (
        f"Model has {total_params:,} parameters, exceeding the 100,000 limit"
    )
    ...
    # --- Save model ---
    model_path = SCRIPT_DIR / "custom_cnn_rf_mod_classifier.pth"
    ...
    plot_confusion_matrix(
        ...
        f"Custom CNN — Training Set Confusion Matrix  (F1={train_f1_final:.4f})",
        SCRIPT_DIR / "custom_cnn_confusion_matrix_train.png",
    )
    ...
    plot_confusion_matrix(
        ...
        f"Custom CNN — Validation Set Confusion Matrix  (F1={val_f1_final:.4f})",
        SCRIPT_DIR / "custom_cnn_confusion_matrix_val.png",
    )
