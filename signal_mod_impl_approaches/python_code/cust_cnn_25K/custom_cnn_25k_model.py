"""
custom_cnn_25k_model.py

Custom lightweight convolutional neural network (~24.9 K trainable parameters)
for RF modulation classification from scaleogram images.

Architecture
------------
    Conv2d(3→8, bias=False)   → BN → ReLU → MaxPool     112×112
    Conv2d(8→16, bias=False)  → BN → ReLU → MaxPool      56×56
    Conv2d(16→32, bias=False) → BN → ReLU → MaxPool      28×28
    Conv2d(32→32, bias=False) → BN → ReLU → MaxPool      14×14
    AdaptiveAvgPool(3×3)                                   3×3×32 = 288
    Dropout(0.3)
    Linear(288→32) → ReLU → Dropout(0.3)
    Linear(32→9)

Conv layers use bias=False since BatchNorm supplies its own learnable bias,
which is standard practice and does not affect model expressiveness.
...
# ---------------------------------------------------------------------------
# Custom CNN model  (~24,913 trainable parameters)
# ---------------------------------------------------------------------------
...
def plot_training_curves(history, save_dir):
    ...
    ax.set_xlabel("Epoch")
    ax.set_ylabel("Loss")
    ax.set_title("Custom CNN 25K — Training and Validation Loss")
    ax.legend()
    ax.grid(True)
    fig.tight_layout()
    fig.savefig(save_dir / "custom_cnn_25k_loss_curves.png", dpi=150)
    plt.close(fig)
    ...
    ax.set_title("Custom CNN 25K — Training and Validation F1 Score")
    ax.legend()
    ax.grid(True)
    fig.tight_layout()
    fig.savefig(save_dir / "custom_cnn_25k_f1_curves.png", dpi=150)
    plt.close(fig)
...
def main():
    ...
    print(f"\nCustom CNN 25K — trainable parameters: {total_params:,}")
    assert total_params <= 25_000, (
        f"Model has {total_params:,} parameters, exceeding the 25,000 limit"
    )
    ...
    # --- Save model ---
    model_path = SCRIPT_DIR / "custom_cnn_25k_rf_mod_classifier.pth"
    ...
    plot_confusion_matrix(
        ...
        f"Custom CNN 25K — Training Set Confusion Matrix  (F1={train_f1_final:.4f})",
        SCRIPT_DIR / "custom_cnn_25k_confusion_matrix_train.png",
    )
    ...
    plot_confusion_matrix(
        ...
        f"Custom CNN 25K — Validation Set Confusion Matrix  (F1={val_f1_final:.4f})",
        SCRIPT_DIR / "custom_cnn_25k_confusion_matrix_val.png",
    )
