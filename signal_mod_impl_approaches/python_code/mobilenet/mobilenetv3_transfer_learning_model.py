"""
mobilenetv3_transfer_learning_model.py

Transfer learning with MobileNetV3-Large for RF modulation classification
...
Architecture notes
------------------
MobileNetV3-Large is a lightweight, efficient CNN designed for mobile and
edge deployment.  Its classifier head is a two-layer Sequential:
    Linear(960, 1280) -> Hardswish -> Dropout(0.2) -> Linear(1280, 1000)
For transfer learning we replace only the final Linear layer so that the
learned 960->1280 projection and activation are preserved.
"""
...
def plot_training_curves(history, save_dir):
    ...
    ax.set_xlabel("Epoch")
    ax.set_ylabel("Loss")
    ax.set_title("MobileNetV3 — Training and Validation Loss")
    ax.legend()
    ax.grid(True)
    fig.tight_layout()
    fig.savefig(save_dir / "mobilenetv3_loss_curves.png", dpi=150)
    plt.close(fig)
    ...
    ax.set_title("MobileNetV3 — Training and Validation F1 Score")
    ax.legend()
    ax.grid(True)
    fig.tight_layout()
    fig.savefig(save_dir / "mobilenetv3_f1_curves.png", dpi=150)
    plt.close(fig)
...
def main():
    ...
    # --- Model: MobileNetV3-Large with modified classifier head ---
    ...
    #     [1] Hardswish()
    #     [2] Dropout(p=0.2)
    #     [3] Linear(1280, 1000)   <-- replace this layer
    #   )
    in_features = model.classifier[3].in_features   # 1280
    model.classifier[3] = nn.Linear(in_features, NUM_CLASSES)
    model = model.to(device)

    print(f"\nMobileNetV3-Large classifier head:")
    ...
    # --- Save model ---
    model_path = SCRIPT_DIR / "mobilenetv3_rf_mod_classifier.pth"
    ...
    plot_confusion_matrix(
        ...
        f"MobileNetV3 Training Set Confusion Matrix  (F1={train_f1_final:.4f})",
        SCRIPT_DIR / "mobilenetv3_confusion_matrix_train.png",
    )
    ...
    plot_confusion_matrix(
        ...
        f"MobileNetV3 Validation Set Confusion Matrix  (F1={val_f1_final:.4f})",
        SCRIPT_DIR / "mobilenetv3_confusion_matrix_val.png",
    )
